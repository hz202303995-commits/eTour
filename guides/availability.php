...
<div class="container">
    <h2>📅 <?= htmlspecialchars(date("F Y", strtotime("$year-$month-01"))) ?></h2>
    ...
    for ($day=1; $day <= $days_in_month; $day++) {
        ...
        // Future clickable (ADD availability)
        else {
            $url = url_with_params(['add'=>$dateStr, 'year'=>$year, 'month'=>$month]);
            echo "<td class='clickable'>
                     <a class='add-link' href='$url'>$content Add</a>
                  </td>";
        }
